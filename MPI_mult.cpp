...
// Заполнение матрицы случайными числами 0..10
...
// Параллельное умножение (результат не сохраняется, только замер времени)
void multiply_mpi(const Matrix& A, const Matrix& B, int rank, int size, int n) {
    ...
    vector<vector<int>> A_local(local_rows, vector<int>(n));
    for (int i = 0; i < local_rows; ++i)
        for (int j = 0; j < n; ++j)
            A_local[i][j] = A_local_flat[i * n + j];

    vector<vector<int>> B_mat(n, vector<int>(n));
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ifstream check(csv_filename);
        bool exists = check.good();
        check.close();
        ofstream out(csv_filename, ios::app);
        if (!exists) {
            out << "size;processes;time_sec\n";
        }
        out.close();
        cout << "MPI experiments\nProcesses: " << size << "\nResults: " << csv_filename << "\n\n";
    }
    ...
    for (int idx = 0; idx < num_sizes; ++idx) {
        ...
        if (rank == 0) {
            ...
            ofstream out(csv_filename, ios::app);
            out << n << ";" << size << ";" << fixed << setprecision(6) << avg_time << "\n";
            out.close();
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
    ...
    if (rank == 0)
        cout << "\nDone. Results saved in " << csv_filename << endl;
